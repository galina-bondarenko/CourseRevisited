...
public class Map extends JPanel {
    ...
    private static final String MSG_WIN_PLAYER1 = "Победил игрок 1!";
    private static final String MSG_WIN_PLAYER2 = "Победил игрок 2!";
    ...
    private void update(MouseEvent e) {
        if (initialized && !isGameOver) {
            ...
            if (isValidCell(cellX, cellY) && isEmptyCell(cellX, cellY)) {
                field[cellY][cellX] = getDotHuman();
                if (!checkGame(getDotHuman(), getStateWinHuman()) && gameMode == GM_HVA) {
                    aiTurn();
                    repaint();
                    checkGame(DOT_AI, STATE_WIN_AI);
                } else if (gameMode == GM_HVH)
                    repaint();
                turn++;
            }
        }
    ...
    private int getStateWinHuman() {
        if (gameMode == GM_HVA || turn % 2 == 0)
            return STATE_WIN_HUMAN;
        else
            return STATE_WIN_AI;
    }

    private int getDotHuman() {
        if (gameMode == GM_HVA || turn % 2 == 0)
            return DOT_HUMAN;
        else
            return DOT_AI;
        ...
        switch (stateGameOver) {
            ...
            case STATE_WIN_AI:
                if (gameMode == GM_HVA)
                    ...
                    g.drawString(MSG_WIN_PLAYER2, 70, getHeight() / 2);
                break;
            case STATE_WIN_HUMAN:
                if (gameMode == GM_HVA)
                    g.drawString(MSG_WIN_HUMAN, 70, getHeight() / 2);
                else
                    g.drawString(MSG_WIN_PLAYER1, 70, getHeight() / 2);
                break;
            default:
    ...
    // Ход компьютера
    private void aiTurn() {
        if(!(turnAIWinCell() || turnHumanWinCell())) {    // проверим, не выиграет ли комп на следующем ходу
            int x, y;
        ...
        for (int i = 0; i < fieldSizeX; i++) {			// ползём по всему полю
            for (int j = 0; j < fieldSizeY; j++) {
                for (int v = 0; v < shift.length; v++) {
                    if (checkLine(i, j, shift[v][0], shift[v][1], winLength, c)) return true;	// проверим линию по х
                }
            }
        }
        return false;
